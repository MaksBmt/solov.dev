...
/**
 * Cursor Distortion — THE.LAB / Cursor.
 *
 * Контент искажается вокруг указателя через SVG feDisplacementMap.
 * Карта смещения рисуется на Canvas: нейтральный серый (128) и
 * радиальный градиент смещения по каналам R/G вокруг курсора.
 */
const POSITION_LERP = 0.14;
const RADIUS_LERP = 0.09;
const DISTORT_RADIUS = 200;
const DISTORT_SCALE = 72;
const LENS_AMOUNT = 0.92;
const MAP_MAX_SIZE = 360;
const SCENE_INIT_MAX_ATTEMPTS = 30;

@Component({
  selector: 'app-cursor-distortion',
  standalone: true,
  imports: [CommonModule, LabDemoLayoutComponent],
  styleUrls: ['./cursor-distortion.component.scss'],
  templateUrl: './cursor-distortion.component.html',
  encapsulation: ViewEncapsulation.None,
})
export class CursorDistortionComponent implements AfterViewInit, OnDestroy {
  ...
  private initScene(): boolean {
    ...
    this.distortionEl = this.sceneEl?.querySelector<HTMLElement>('.js-distortion') ?? null;
    this.mapCanvas = this.sceneEl?.querySelector<HTMLCanvasElement>('.js-distortion-map') ?? null;
    this.mapCtx = this.mapCanvas?.getContext('2d', { willReadFrequently: true }) ?? null;
    this.feImageNode = this.sceneEl?.querySelector<SVGImageElement>('.js-distortion-feimage') ?? null;
