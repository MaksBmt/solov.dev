...
export class MorphMenuComponent implements AfterViewInit, OnDestroy {
  ...
  private syncTarget(index: number) {
    ...
    if (!item || !this.sceneEl) return;

    const sceneRect = this.sceneEl.getBoundingClientRect();
    const rect = item.getBoundingClientRect();
    const pad = 6;

    this.targetX = rect.left - sceneRect.left - pad;
    this.targetY = rect.top - sceneRect.top - pad;
  ...
  drawDebug({ ctx, width, height }: { ctx: CanvasRenderingContext2D; width: number; height: number }) {
    ...
    ctx.lineWidth = 1;
    ctx.setLineDash([4, 4]);
    ctx.strokeRect(this.currentX, this.currentY, this.currentW, this.currentH);
    ctx.setLineDash([]);
